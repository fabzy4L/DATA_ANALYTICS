...
#1A. what is the probability of 8 black socks in a 12 sock sample.
...
```
```{r}
#1B. Probability of grabbing 8 or more black individual socks (4 pairs or more), from an 18 sock random sample.
...
```
```{r}
#1C. How many socks do I need to pick to make sure I at least grab 6 black socks (3 pairs) from a 30 random sample, with a Chance of 50% success? - Between 18 and 19 socks must be grabbed for a 41 - 59% chance of succesfully grabbing 3 pairs.
...
```{r}
#4. Was there an effect to changing dietary regimen from HIGH FAT-HIGH SALT to DIET? i.e. BEFORE ESTABLISHING A DIET AND AFTER ESTABLISHING A DIET.
SBP.exam
```
...
```{r}
#PLOT OF BP CHANGES BEFORE ESTABLISHING A DIET AND AFTER ESTABLISHING A DIET:
plot(SBP.exam$Individual, SBP.exam$SBP.before, ylab = c("SBP"), xlab= c("Individuals"), ylim = c(100,145), xlim=c(1,10))

points(SBP.exam$Individual, SBP.exam$SBP.before)
points(SBP.exam$Individual, SBP.exam$SBP.after, col='red', pch=8)
legend("topright", c("Before","After"),pch=c(1,8),col=c("black","red"), cex=0.7)

```


```{r}
# I modified the CSV file to make ANOVA easier to produce (also, it had an error), which allowed for me to perform:
# Two Step Analysis - ANOVA and POST-HOC TUKEY FOR HONESTLY SIGNIFICANT DIFFERENCE.
# ANOVA showed no significant difference in BP among either groups, BEFORE and AFTER implementing a dietary regimen. Neither the group nor the individuals had an influence separately, nor they interacted in any significant way (p>0.05)
# Overall, the difference among values reported between the groups was not significant eiher, with a P = 0.37732.
...
```{r}
# Two Step Analysis - ANOVA and POST-HOC TUKEY FOR HONESTLY SIGNIFICANT DIFFERENCE.
# Upon ANOVA and TUKEY TEST ANALYSIS, it was made evident that no group had a particular advantage in comparise to the rest, in terms of accelerating sleep descent into REM.
...
```
```{r}
# Plotbox graph, comparing times until REM. 
...
```
```{r}
#6A. Create a formula to predict concentration of oil at a given distance. 
# Tested both quadratic and cubic formulas as well to compare fit vs the linear regression model and noticed no actual improvement whatsoever when in comparison. Additionally, quadratic formula did not show any particular statistical significance when comparing the additional quadratic component to the linear one: I(gw.wells$Distance.m^2) -- P = 0.833.

...
#lines(B2$mm[order(B2$mm)], predict(cubic.B2)[order(B2$mm)], col="black")
lines(B2$mm[order(B2$mm)], predict(cubic.B2.2)[order(B2$mm)], col="black", lty=c(1), lwd=1.5)
#abline(lm(ybase~xbase), lty=2)
...
points(4.32,17,pch=8,cex=1, col = c("black"))
text(6,40,"E ~ 5 kPa")
legend("topright", c("Failure"),pch=8,col=c("black"), cex=0.7)
```
...
```{r}
plot(A1$mm,corr.y.A1, xlim=c(0,7.25), ylim = c(0,50), type = 'n', ylab = "", xlab = "", font=2, cex.axis=0.8 )

mtext(side=1, line=2.3, "Displacement (mm)", col="black", font=2, cex=1.2)
mtext(side=2, line=2.3, "Stress (kPa)", col="black", font=2, cex=1.2)
...
lines(B1$mm[order(B1$mm)], predict(cubic.B1.2)[order(B1$mm)], col="red", lwd = 2, lty=c(1))
abline(lm(ybase~xbase), lty=2)
points(5.060,38,pch=8,cex=1, col = c("black"))

text(5.75,45,"E ~ 9 kPa")

legend("topleft", c("1 Week","3 Weeks"), lty=c(1),col=c("red", "black"), cex=0.7, lwd=2)

lines(B2$mm[order(B2$mm)], predict(cubic.B2.2)[order(B2$mm)], col="black", lty=c(1), lwd=2)

abline(lm(y2.5~xbase), lty=2)
points(4.32,17,pch=8,cex=1, col = c("black"))
text(5.75,23.5,"E ~ 5 kPa")
legend("topright", c("Failure"),pch=8,col=c("black"), cex=0.7)




complex.mod <- function(x,y){
  complex <- ((((x^2)+(y^2)))^(1/2))
  return(complex)
}

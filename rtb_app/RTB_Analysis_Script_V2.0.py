...
def clean_dataset(df):
    ...
    for col in ["Hours", "Units", "UPH", "LC"]:
        if col not in df.columns:
            df[col] = pd.NA
    for col in ["Hours", "Units", "UPH"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df["LC"] = df["LC"].apply(normalize_lc)
    df = df.dropna(subset=["Hours", "Units", "UPH"])
    df = df[(df["Hours"] > 0) & (df["Units"] > 0) & (df["UPH"] > 0)]
    return df
...
def main():
    ...
    print(f"[rtb] csv_path={args.csv_path}")
    if not os.path.exists(args.csv_path):
        print(f"[rtb] ERROR: CSV not found at: {args.csv_path}")
        return

    try:
        lc_df, comment = analyze_file(args.csv_path)
        print("[rtb] analysis complete\n")
        print(comment)  # keep console output

        # Save LC summary CSV (UTF-8)
        lc_df.to_csv("LC_summary.csv", index=False, encoding="utf-8", lineterminator="\n")
        print("[rtb] wrote LC_summary.csv")
        ...
        with open(out_path, "w", encoding="utf-8", newline="\n") as f:
            f.write(comment)
        print(f"[rtb] wrote comment -> {out_path}")

    except Exception:
        print("[rtb] ERROR during analysis:\n" + traceback.format_exc())
